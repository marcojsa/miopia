-- ============================================================
-- supabase_test_helpers v0.0.6 — VENDORIZADO de
-- https://github.com/usebasejump/supabase-test-helpers
-- (arquivo original: supabase_test_helpers--0.0.6.sql)
--
-- Adaptações para rodar via `supabase test db` (pg_prove + psql):
--  1. Removida a guarda `\echo ... \quit` de extensão (o arquivo é
--     executado direto pelo psql, não via CREATE EXTENSION).
--  2. Adicionado `create extension if not exists pgtap` (idempotente).
--  3. Bloco TAP mínimo no FINAL do arquivo, como o próprio README do
--     basejump recomenda, para o pg_prove aceitar o arquivo.
--  4. Adicionado `create extension if not exists "uuid-ossp"`:
--     tests.create_supabase_user() chama extensions.uuid_generate_v4().
-- Nenhuma função foi alterada.
-- ============================================================

-- pgTAP vem na imagem do supabase/postgres; garantir que está instalado.
create extension if not exists pgtap with schema extensions;
-- uuid-ossp: usado por tests.create_supabase_user (uuid_generate_v4)
...
-- ============================================================
-- Bloco TAP mínimo (adaptação 3): o pg_prove exige que TODO arquivo
-- de teste emita um plano TAP — sem isto o `supabase test db` falha
-- com "no plan found" neste arquivo, que existe só pelos helpers.
-- ============================================================
begin;
select plan(1);
select ok(true, 'helpers de teste (schema tests.*) instalados');
select * from finish();
rollback;